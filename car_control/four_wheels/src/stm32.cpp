...
bool decode()
{
	...
	//ROS_INFO("decode %s",recvBuffer.c_str());
...
{
	ros::init(argc, argv, "cmdvel2stm32");
	ros::NodeHandle n;
	...
	//subscribers
	ros::Subscriber cmd_vel_sub = n.subscribe("cmd_vel", 2, onCmdVel);
	...
	ros::Time last_time = ros::Time::now();
  	while (ros::ok() && !shutdown_flag)
  	{
		...
		if (odom_received)
		{
	    	...
	    	if (last_deviceState!=deviceState)
	    	{
